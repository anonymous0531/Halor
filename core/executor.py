...
class Executor(object):
    ...
    # def training_handler(self, clientId, conf, resample_count):
...
#        train_res['global_model_delta_acc'] = self.global_model_delta_acc
    ...
    # def testing_handler(self, args, portion_count):
        """Test model"""
        evalStart = time.time()
        device = self.device
        ...
        # data_loader = select_dataset(self.this_rank, self.testing_sets, batch_size=args.test_bsz, isTest=True, collate_fn=self.collate_fn, vector = portion_count)
        ...
        while True:
            if not self.server_event_queue.empty():
                ...
                # initiate each training round
                    ...
                    # clientId, client_conf, resample_count_vector = event_dict['clientId'], self.override_conf(event_dict['conf']), event_dict['count']
                    # logging.info(f"excutor.py: Executor {self.this_rank}: Client {clientId}, resample_count_vector is {resample_count_vector}")    #stage3 test
                    ...
                    # train_res = self.training_handler(clientId=clientId, conf=client_conf, resample_count = resample_count_vector)                    
                    ...
                    # logging.info(f"excutor.py: Executor {self.this_rank}: Client {clientId}, training finish")    #stage3 test
                elif event_msg == 'test':
                    test_res = self.testing_handler(args=self.args)
                    
                    # dynamic_global_distribution = event_dict['dynamic_global_distribution']
                    # test_res = self.testing_handler(args=self.args, portion_count = dynamic_global_distribution)
